...
### Backend / API
- **BE-01 (re-scoped 2026-05-27):** ~~"доверяем клиентскому transaction_id"~~ устарело — `payments/apple/verify.go` уже **полностью** валидирует StoreKit 2 JWS: подпись против Apple root CA (x5c chain), bundle id, environment (Production/Sandbox), product id whitelist, revoked flag, expired guard (H-008), charge_id idempotency. **Реальный остаток:**
  - **BE-01a (P0, backend half landed 2026-05-27):** `appAccountToken` extracted from JWS в `apple.Verify` → `Transaction.AppAccountToken` → logged в `subscription verified` событии (фичи `app_account_token_present`, `app_account_token`). Это telemetry-only пока — соберём адопшн до flip'а в strict-reject mode (по аналогии с MED-012 install_secret rollout). **iOS-сторона осталось:** `SubscriptionManager.purchase` должен передать `Product.PurchaseOption.appAccountToken(uuid)` где uuid = детерминированный UUID5 от user.id (тот же namespace на backend для match'а). После ≥95% adoption → backend reject на mismatch.
  - ✓ **BE-01b Phase 1 (2026-05-27):** `internal/asc` package — ES256 JWT-signed клиент к App Store Connect API (`/v1/apps`, `/v1/builds`, `/v1/apps/.../appStoreVersions`, `/inAppPurchasesV2`). 5-минутный in-memory cache + singleflight чтобы при 5 одновременных админах не зажечь Apple rate-limit (3500 req/h). Endpoint `GET /api/v1/admin/status/apple` отдаёт versions/IAPs/builds для Status page. SPA рендерит секцию "Apple App Store state" с цвет-кодированными state badges (READY_FOR_SALE green, WAITING_FOR_REVIEW amber, REJECTED red). `asc.New()` returns nil без credentials — admin показывает placeholder вместо 500. **Остаётся (Phase 2 / true BE-01b ops):** App Store Server API v2 (transaction lookup, refund history, восстановление missed ASN v2 notifications) — нужен **отдельный** ASC API key (Users → Integrations → **In-App Purchase** role).
...
- ✓ **MON-06 Phase 1 (2026-05-27):** Singbox handshake-failure aggregation. `scripts/singbox-log-watcher.py` (Python, cron */1) парсит `docker logs --since 65s singbox`, регулярка ловит `REALITY: processed invalid connection from <IP>:<port>`, агрегирует per-IP и пишет один-line-per-minute JSONL в `/var/log/singbox-events.jsonl`. Backend `GET /api/v1/admin/status/handshake-errors?hours=N` читает файл, делает hourly buckets (calendar-padded), top-10 IPs + watcher freshness flag (last tick > 5min = "watcher down").
- ✓ **MON-06 Phase 2 (2026-05-27):** Bot/user correlation. Watcher делает batched `docker exec chameleon-postgres psql ... WHERE last_ip IN (...)` для каждого tick — IPs которые match'ат `users.last_ip` отмечаются `is_user: true`. JSONL расширен: `users: {ip → vpn_username}`, `user_errors`, `bot_errors`. Backend суммирует разделённо. SPA: two-column split (real users in red ← actionable, bot probes muted grey ← noise), stacked-bar chart с user-errors поверх bot-errors, dedicated "Affected users" таблица первой, "Top sources (all)" свёрнут в `<details>`. Backward-compat для Phase 1 JSONL lines (без `users` map → fall back to "all bots"). Текущий NL volume: ~300/h, после фильтра обычно 0-2 real user failures, остальное — internet :443 scanners.
...
- ✓ **MON-08 (2026-05-27):** Service Status admin page (`/admin/app/status`). Aggregate endpoint `GET /api/v1/admin/status` запускает probes в параллель с per-probe 3s timeout. **Services**: postgres (pg.Pool.Ping + pool stats), redis (PING), singbox-tls (TCP dial :443), chameleon-api. **Integrations**: cloudflare-edge madfrog.online, msk-relay, spb-relay, apple-asc-api (HTTP 401 = reachable+unauth), apple-storekit, freekassa. **Recent events**: последние 20 admin_audit_log записей с цвет-кодингом action. Auto-refresh 30s, overall-status badge (all-OK / service-degraded / integration-warn). Sidebar entry "Status" в Overview.
...
- ✓ **USR-04 (2026-05-27):** Pagination в admin SPA. Page/PageSize state + reset-to-1 на change поиска/сорта/page-size, controls: First/Prev/Next/Last + selector 25/50/100/200 + counter "X–Y of Z". `placeholderData: prev` сохраняет старую страницу пока next грузится — нет flicker. **Virtual scroll** не сделан — при 200 строк не нужен; вернуться когда total > 5к.
...
- ✓ **USR-06 / USR-09 Phase 1 (2026-05-27):** Funnel page `/admin/app/funnel`. Backend `db.FunnelSeries(days)` + `GET /api/v1/admin/stats/funnel?days=` строит из существующих `users` + `payments` (без iOS работы): signups_per_day, dau_per_day (calendar-padded so aligned), auth provider mix, conversion (signups → paid Apple/FreeKassa, **admin grants excluded**), avg days to convert, 4-week retention cohort matrix. SPA: 4 stat cards + Signups/DAU line chart + auth provider horizontal bars + cohort matrix с heat-coloring (green ≥60% / yellow ≥20% / red >0). Recharts (lazy bundle 383KB). 60s refresh. Integration test `TestFunnelSeriesShape` пинит calendar padding + admin-exclude + cohort weeks. **Phase 2 (iOS events)** остался: paywall.view / product.tap / purchase.cancel / connect.fail.
- ✓ **USR-07 (2026-05-27):** Top-N traffic outliers. Backend `db.TopTrafficUsers(days, limit)` — SUM(used_traffic) в окне с LEFT JOIN на users (orphaned vpn_username'ы тоже видим). Endpoint `GET /api/v1/admin/stats/traffic-outliers?days=&limit=`, clamps days[1..90] limit[1..100]. Admin dashboard — новый widget "Top traffic" с переключателем 24h/7d/30d/90d, флаги, GB, active/expired badge. Integration test `TestTopTrafficUsers` пинит порядок sort + LEFT JOIN orphan + window clamping. **Гэп «avg GB/month per user»** оставлен на USR-06 (activity charts) — outliers закрыты.
- ✓ **USR-08 (2026-05-27):** Audit log viewer в admin SPA — раздел "Activity" под /admin/app/audit. Backend `ListAuditEvents` с filter struct (admin_id, action, since, until) + pagination, LEFT JOIN admin_users для username. `ListAuditActions` возвращает distinct actions за 90д для dropdown'а. UI: фильтры action/admin/since/until + table + pagination как в users. Цвет action по семантике (login.success/restart → green/cyan, delete/.failed → red). Integration test `TestListAuditEventsFilters` пинит filter combinations + LEFT JOIN. Visible to admin/operator/viewer — table holds no secrets (auditSafeUsername sanitize from MED-014).
- ✓ **USR-09 Phase 2 backend + admin (2026-05-28):** iOS client-side event tracking — closes the pre-purchase blind spot Phase 1's SQL-derived funnel couldn't see (paywall views, product taps, purchase cancels, vpn connect failures).
  - **Backend:** migration `017_app_events.sql` (id, user_id FK, device_id, event_name, properties JSONB, app_version, platform, occurred_at, received_at, ip, country; three indexes for user-detail / per-name-timeseries / generic-feed reads). `db.InsertAppEvents` batch INSERT with JSONB round-trip + cascade-on-user-delete; `db.ListAppEvents` filtered+paginated; `db.CountAppEventsByNameDaily` for the chart; `db.DistinctEventNames` for the dropdown. `POST /api/v1/mobile/events/batch` (JWT-gated, 100 events / 64KB cap, ISO8601 client clock parsed + clamped to ±90d, event_name regex `^[a-z][a-z0-9._-]{0,63}$`, server enriches ip/CF-IPCountry/app_version/platform, malformed JSON / DB errors → 200 with accepted:0 so iOS never sees a transient backend issue). Admin reads under `/api/v1/admin/events{,/counts,/names}`.
  - **iOS (shipped together, awaits next build):** `EventTracker` actor — in-memory queue (cap 500, drop-oldest), batch flush (cap 100 per request), persists JSON to Application Support on flush, restores on launch, drops events older than 7d. Fires on app foreground/background AND every 5 min while active. Call sites: `app.launch` / `app.foreground` / `app.background` (lifecycle), `paywall.view` (PaywallView.task), `paywall.product.tap` (plan card tap), `purchase.start` / `purchase.success` / `purchase.cancel` / `purchase.fail` (paywall buy button), `vpn.connect.start` / `vpn.connect.success` / `vpn.connect.fail` (toggleVPN + watchdog outcomes). `AppState` owns the tracker via lazy init that captures Bundle version + PlatformDevice.identifier once. Tests: `EventTrackerTests.swift` covers buffer-without-send, drain-on-success, keep-on-failure, persist+restore across "process restart".
  - **Admin SPA:** `/admin/app/events` page — top stacked-bar chart (14d, name × day, inline SVG, no chart-lib dep), filter strip (event_name dropdown / user_id / since-until UTC / page-size 25-200), table (time, user, name pill colour-coded by hash, properties JSON preview ≤120ch, ip, country, version), pagination footer mirroring `/audit`. Sidebar entry under Overview.
  - **⚠ App Privacy disclosure required before submitting next build that includes the iOS half.** See `docs/LAUNCH_CHECKLIST.md` LAUNCH-04. Without updating ASC's "App Privacy" data-types before submission Apple will reject under Guideline 5.1.1 (data collection mismatch).
  - **CI fix shipped alongside:** `clients/admin/eslint.config.js` (flat config for ESLint 9) + `backend/.golangci.yml` (v2 conservative set: govet/ineffassign/unconvert/misspell) + pinned `golangci-lint-action@v7` with `version: v2.12.2`. Closes the stage-b CI lint hole that's been red since 1102da8 (2026-04-23). Pre-existing tech-debt catalogued as TD-LINT-01/02 below.
...
- ✓ **Apple (verified via ASC API 2026-05-27 wave 2):** 1.0.26 build 75 `READY_FOR_SALE` 175 territories. Build 89 (1.0.27) Internal TF `IN_BETA_TESTING`, External TF `WAITING_FOR_BETA_REVIEW` (с 2026-05-26 21:42 UTC). 4 IAPs (sub.30/90/180/365days) submitted via `POST /v1/inAppPurchaseSubmissions`, state `WAITING_FOR_REVIEW`. 2 Resolution Center threads acknowledged. **Без approved IAPs paywall в 1.0.26 показывает empty StoreKit products = ноль выручки на проде.**
- ✓ **App Store v1.0.27 submitted (2026-05-27 19:18 UTC):** новая `appStoreVersion` `857045e1-...` создана через ASC API, build 89 привязан, ru+en-US whatsNew заполнен (Widgets / Control Center / Shortcuts / Apple+Google SSO стабильнее / security audit). reviewSubmission `7f541058-...` state `WAITING_FOR_REVIEW`. ETA 24-48ч.
