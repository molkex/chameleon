...
- **BE-01 (re-scoped 2026-05-27):** ~~"доверяем клиентскому transaction_id"~~ устарело — `payments/apple/verify.go` уже **полностью** валидирует StoreKit 2 JWS: подпись против Apple root CA (x5c chain), bundle id, environment (Production/Sandbox), product id whitelist, revoked flag, expired guard (H-008), charge_id idempotency. **Реальный остаток:**
  - **BE-01a (P0):** `appAccountToken` binding — iOS должен передавать `Product.PurchaseOption.appAccountToken(user_uuid)` при `product.purchase()`, backend проверять что token из JWS совпадает с authenticated user.id. **Без этого:** утёкший JWS можно replay'нуть на чужой аккаунт (charge_id idempotency защищает от двойного credit'а на ОДНУ запись, но не от credit'а на ДРУГОГО юзера). iOS-сторона нужна одним коммитом (`SubscriptionManager.purchase` + persistent UUID per device_id). Backend: парсинг + soft-warn → strict после ≥95% iOS adoption (как с MED-012).
