...
- **🚀 Phase 1 (partial) — first-write callback** (build 56, archive ready 2026-05-13): порт `FirstWriteCallbackConn` паттерна mihomo в `sing-box-fork` (`common/callback/firstwrite.go` + `protocol/group/urltest.go`). Закрывает DPI handshake-kill: TCP/TLS dial OK + probe gstatic 204 OK, но первая user-Write роняется DPI → теперь сразу invalidate URL test history → reselect на следующем probe. До этого outbound оставался выбранным на полный probe interval (5s-3min). Libbox.xcframework v1.13.6 (минус tailscale/naive/dhcp для 50MB jetsam cap). Tests: `common/callback/firstwrite_test.go` (4/4 кейса). Build 55→56, архив в `/tmp/MadFrogVPN-build56.xcarchive`, upload в TestFlight pending user OK.
