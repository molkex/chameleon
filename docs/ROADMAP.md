...
- **✅ Phase 1.C — Adaptive probe cadence** (code merged 2026-05-13 22:55, commit 6005fa5; awaits build 61): `TunnelStallProbe.applyCellularProfile()` / `applyWiFiProfile()` switch tuneables in response to `NWPath.isExpensive` from `ExtensionPlatformInterface.handlePathUpdate`. Cellular = threshold 1 / interval 5s (fallback in ~10s of throttle); Wi-Fi keeps 2/15 (avoid false-positive blips). Probe loop reads `config.probeInterval` per-iteration, mid-flight swap takes effect on next tick. `var config` (was `let`). Build verified via `xcodebuild build` on iOS sim — unit tests blocked by sim's missing App Group entitlement (pre-existing). Pending: bump `project.yml::CURRENT_PROJECT_VERSION` 60→61 + ASC upload. Deferred until user confirms 60.3 alone resolved «то грузит то не грузит».
- **⏳ Phase 1.D — Penalty score for throttled outbound** (deferred, требует Clash API extension): после `invokeFallback()` метить активный outbound 60-секундным штрафом, decay линейно. mihomo имеет `proxies/:name/delay`, но penalty injection нужно дописать.
