...
- ✓ **MON-06 Phase 1 (2026-05-27):** Singbox handshake-failure aggregation. `scripts/singbox-log-watcher.py` (Python, cron */1) парсит `docker logs --since 65s singbox`, регулярка ловит `REALITY: processed invalid connection from <IP>:<port>`, агрегирует per-IP и пишет один-line-per-minute JSONL в `/var/log/singbox-events.jsonl`. Backend `GET /api/v1/admin/status/handshake-errors?hours=N` читает файл, делает hourly buckets (calendar-padded), top-10 IPs + watcher freshness flag (last tick > 5min = "watcher down"). SPA Status page получает sparkline-bar chart per-hour + top sources table. Цвет per-bar по плотности (green→yellow→red). Текущий volume на NL: ~300 errors/h, большинство — bot-зонды на :443 (185.218.0.43 SPB forwarder, 85.140.3.85 RU residential, 85.239.49.28 старый MSK bot host). **Phase 2 (отложил):** correlation с users.last_ip чтобы помечать real-user failures vs bot probes; structured field `device_id` если sing-box логирует known-user attempts отдельно.
