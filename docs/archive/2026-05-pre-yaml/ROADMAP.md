...
- **❌ Phase 0.1 — Telegram direct routing (REVERTED 2026-05-13 16:23)**: попытка маршрутизировать Telegram домены+CIDRs через `RU Traffic` селектор (direct в Smart/Split-tunnel). **Сломалось у юзеров на сетях где Telegram CDN IPs блокируются direct** — TCP `dial: i/o timeout` на 91.105.192.x / 149.154.x / 95.161.76.x. Field log 2026-05-13 16:21 показал реальный кейс. Откатил коммит `dba46b8`. Урок: предположение «Telegram unblock'нут в РФ, direct работает» неверно — у части юзеров (мобильные операторы, локальные ISP) Telegram CDN режется на TCP-уровне, нужен VPN. Правильный путь: data-plane health (Phase 1) который **сам** обнаружит unreachable IP'ы и переключит на VPN.
- **✅ Phase 1 — first-write callback** (build 56→60, done 2026-05-13): порт `FirstWriteCallbackConn` паттерна mihomo в `sing-box-fork` — `common/callback/firstwrite.go` + патч `protocol/group/urltest.go`. Закрывает DPI handshake-kill: TCP/TLS dial OK + probe gstatic 204 OK, но первая user-Write роняется DPI → сразу invalidate URL test history → reselect на следующем probe. Builds 56-58 шипились на libbox v1.13.6 — OOM regression на iOS NE (memory pressure → oom-killer reset loop, см. Phase 1.9). Build 60 откатил libbox на v1.13.5 + cherry-pick callback patch (нет конфликтов).
- **✅ Phase 1.5 — throughput-time threshold** (build 57, done): `Shared/TunnelProbeOutcome.swift` — pure `(statusOK, bytes, elapsedMs) → .healthy | .throttled | .failed` классификатор. `PacketTunnel/TunnelStallProbe.swift` ре-инклудит active fallback на degraded outcomes. Ловит DPI **throttle** (full body, медленно).
- **✅ Phase 1.6 — App Review re-fix** (build 59, done): SIWA вернулась на native SwiftUI `SignInWithAppleButton` (iOS 26 multi-scene anchor lookup), modal `.alert()` для ошибок auth, breadcrumbs, accessibility id. App Review 2.1(a) iPad Air M3 fix.
- **✅ Phase 1.7 — Routing mode UX** (build 58, done): default `.fullVPN → .ruDirect`, новое `RoutingMode.recommended`. «Умный» переименован в «Только блоки» с предупреждением про cellular throttle. Segments reordered `[ruDirect, fullVPN, smart]`.
- **✅ Phase 1.9 — Libbox memory regression** (build 60, done): откат libbox на sing-box v1.13.5 + cherry-pick callback patch. `LibboxVersionGuardTests` (2 теста) пинит версию чтобы регрессия не повторилась. iOS-arm64 binary 44MB = production build 55.
- **✅ Phase 1.A — Suppress QUIC outbounds** (backend 60.2-no-quic-outbounds, done 2026-05-13 22:15): env-flag `CHAMELEON_DISABLE_QUIC_OUTBOUNDS=true` в `backend/docker-compose.yml`, читается в `clientconfig.go` per-call (tests flip через `t.Setenv`). Подавляет h2/tuic outbound'ы в mobile config — каждый ~2-3 MiB на Go heap (BBR congestion state + uTLS handshake) у iOS NE 50 MB jetsam cap. Field log после деплоя: **0 oom-killer events** (было 103218 за 4 минуты предыдущего run), **6 outbound leaves init** вместо 10. Phase 1.9 откат на v1.13.5 был **wrong call** — реальная причина OOM не в v1.13.6 → v1.13.5 регрессии (single-line iovec change безобиден для freshly-created buffers), а в конфиге размером 44 outbound'а. Tests: `TestDisableQUICOutboundsEnvSuppressesH2AndTUIC` + `TestDisableQUICOutboundsEnvOffKeepsQUIC` (guard от silent flip default).
- **✅ Phase 1.B — DPI-aware config filter** (backend 60.3-dpi-aware-config, done 2026-05-13 22:50): `ClientConfigOpts.ClientCountryCode` прокинут end-to-end. `mobile/config.go::resolveClientCountry(c)` зовёт `geoip.Lookup(c.RealIP())` с 1s timeout (free ip-api.com, safe-empty на miss). `clientconfig.go::clientNeedsDPIBypass()` проверяет country против `dpiBlockedDirectCountries{RU, BY}` — для RU/BY клиентов skip `de-direct-*` leaf генерация (162.19.242.30 OVH Frankfurt ASN-блокируется всеми RU/BY ISP — `context deadline exceeded` каждые 15с в urltest cycle = «висит каждые 15с на 5с»). NL Timeweb остаётся (throttled но reachable); `de-via-msk` chain остаётся (intended DE exit path внутри DPI envelope). 5 тестов: RU→no-de-direct, BY→no-de-direct, US→full, empty→full, case-insensitive. CF→nginx→Echo IP pipeline уже даёт реальный client IP через `CF-Connecting-IP` (commit b7f09c2 закрыл XFF spoof).
- **✅ Phase 1.C — Adaptive probe cadence** (build 61 shipped, field-verified 2026-05-13 23:33 LTE): `TunnelStallProbe.applyCellularProfile()` / `applyWiFiProfile()` switch tuneables in response to `NWPath.isExpensive` from `ExtensionPlatformInterface.handlePathUpdate`. Field log shows `[network] pathUpdate expensive=true` → `cellular profile applied (threshold=1, interval=5s)` → `probe THROTTLED(1735ms) → degraded #1 → STALL streak — invoking fallback → urlTest('Auto') OK` — fallback latency THROTTLED→urlTest **9 ms** (vs ~21 s on build 60). Architecture nuance: `pathUpdateHandler` fires multiple times on stable uplink (5+ in 22 s) — initial race-condition fear was unfounded, the second callback always lands.
- **✅ Phase 1.C polish — log quality** (build 62): replaced `applyCellularProfile/applyWiFiProfile` pair with single `apply(_ profile: Profile)` that dedupes on no-op. Field log now reads as narrative: `profile initial → cellular (threshold=1, interval=5s)` on first apply, no spam on subsequent re-confirmations. Added explicit `probe recovered after N degraded — counter reset` event so silent counter-zeroing is visible. `start()` logs the starting profile + first-probe delay for full startup context. `Profile` enum on the type with computed `stallThreshold` / `probeInterval` so adding new profiles (e.g. `.ethernet` for tethered Mac) is one case statement, not parallel functions.
- **⏳ Phase 1.D — Penalty score for throttled outbound** (NEXT, iOS-only, no fork): after `STALL streak — invoking fallback`, capture currently-active outbound tag (`LibboxCommandClient.GetGroupItems`), persist a per-outbound "do not re-select until T" map (Swift-side), and call `client.selectOutbound(group, name)` to pin the first non-penalised member — bypasses urltest's re-probe that currently re-elects the same throttled outbound after seconds. 60 s decay then resumes normal urltest cycle. Closes the throttle→fallback→same-outbound loop the user still experiences every ~90 s on a single leg.
- **⏳ Phase 1.E — Server-side request rotation** (research, deferred): tear down and re-dial upstream VLESS Reality every ~75 s in fork so DPI sees fresh small flows instead of one long bulk flow. Memory risk vs 50 MB jetsam cap — needs heap instrumentation before commit.
- **⏳ Phase 1.5 — close-reason failure counter** (next): mid-flight DPI kills (Telegram-медиа сценарий из field log 2026-05-13) — отдельный механизм, нужен sliding-window counter по `EOF`/`RST` в течение N секунд от dial.
- **⏳ Phase 2 — Smart LightGBM scoring** (далее): port full mihomo Smart group с ML-моделью + MAD anomaly detection. Не переизобретаем.
