...
//
// clientBuild is the client's self-reported build number (0 if unknown/old
// client) — see leanMode / firstUrltestSafeBuild in clientconfig.go for how
// it gates the urltest vs. lean-selector config shape.
//
...
func (e *SingboxEngine) GenerateClientConfig(user VPNUser, servers []ServerEntry, chains []ChainedEntry, clientBuild int) ([]byte, error) {
	...
	return generateClientConfig(e.cfg, user, servers, chains, clientBuild)
}
