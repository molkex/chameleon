...
// TestTelegramRoutesViaRUTraffic asserts that Telegram domains and CIDRs are
// routed through the "RU Traffic" selector — NOT hardcoded direct, NOT
// hardcoded Proxy. The rationale (build 56.2, 2026-05-13): Telegram CDN IPs
// (e.g. 91.105.192.100, 95.161.76.100, 149.154.167.x) are physically in
// NL/UK per ip-api.com — registered to Telegram Messenger Inc, not Russian
// ISPs — so the geoip-ru ruleset doesn't match them. Without an explicit
// rule, Telegram falls through to Default Route which in Split-tunnel mode
// is Proxy. Result: user RU → NL VPN exit → NL Telegram CDN. Extra ~30-50ms
// RTT vs direct, noticeable on media (photos/videos).
//
// Routing through "RU Traffic" selector instead of direct preserves user
// intent across modes:
//   - Smart / Split-tunnel (RU Traffic=direct): Telegram direct → fast
//   - Full-VPN (RU Traffic=Proxy): Telegram via VPN → user explicit choice
//
// Implementation note: sing-box uses AND between fields within a single rule,
// so we emit TWO rules — one by domain_suffix, one by ip_cidr — to get OR
// semantics (Telegram matches by either domain or IP).
//
// Both rules must sit BEFORE the generic ".ru" / geoip-ru rules so Telegram-
// specific routing wins. They sit AFTER ruAlwaysDirectDomains so the
// always-direct shortlist still wins for things like Sberbank.
func TestTelegramRoutesViaRUTraffic(t *testing.T) {
	cfg := parseGenerated(t)
	rules, ok := cfg["route"].(map[string]any)["rules"].([]any)
	if !ok {
		t.Fatal("route.rules missing or wrong type")
	}

	domainRule, domainIdx := findTelegramRule(rules, true /*byDomain*/)
	ipRule, ipIdx := findTelegramRule(rules, false /*byCIDR*/)

	if domainRule == nil {
		t.Fatal("no route rule covering Telegram domains found (need domain_suffix rule)")
	}
	if ipRule == nil {
		t.Fatal("no route rule covering Telegram CIDRs found (need ip_cidr rule)")
	}
	if domainRule["outbound"] != "RU Traffic" {
		t.Errorf("Telegram domain rule outbound=%v, want \"RU Traffic\"", domainRule["outbound"])
	}
	if ipRule["outbound"] != "RU Traffic" {
		t.Errorf("Telegram CIDR rule outbound=%v, want \"RU Traffic\"", ipRule["outbound"])
	}

	// Precedence: both Telegram rules must come BEFORE geoip-ru / refilter so
	// they win over the catch-all rules.
	for i, r := range rules {
		m, ok := r.(map[string]any)
		if !ok {
			continue
		}
		if m["rule_set"] == "geoip-ru" || m["rule_set"] == "refilter" {
			if i < domainIdx {
				t.Errorf("Telegram domain rule at idx %d, but %v at idx %d — must come first",
					domainIdx, m["rule_set"], i)
			}
			if i < ipIdx {
				t.Errorf("Telegram CIDR rule at idx %d, but %v at idx %d — must come first",
					ipIdx, m["rule_set"], i)
			}
		}
	}
...
// TestTelegramRuleCoversKnownDomainsAndCIDRs asserts that the Telegram route
// rules cover the public Telegram CIDR list (core.telegram.org/resources/cidr.txt)
// plus client-facing domains.
func TestTelegramRuleCoversKnownDomainsAndCIDRs(t *testing.T) {
	cfg := parseGenerated(t)
	rules, ok := cfg["route"].(map[string]any)["rules"].([]any)
	if !ok {
		t.Fatal("route.rules missing")
	}
	domainRule, _ := findTelegramRule(rules, true)
	ipRule, _ := findTelegramRule(rules, false)
	if domainRule == nil || ipRule == nil {
		t.Fatal("Telegram rules not found")
	}

	wantDomains := []string{"t.me", "telegram.org"}
	gotDomainsRaw, _ := domainRule["domain_suffix"].([]any)
	var gotDomains []string
	for _, d := range gotDomainsRaw {
		if s, ok := d.(string); ok {
			gotDomains = append(gotDomains, s)
		}
	}
	for _, d := range wantDomains {
		if !slices.Contains(gotDomains, d) {
			t.Errorf("Telegram domain rule missing %q; got %v", d, gotDomains)
		}
	}
	...
	wantCIDRs := []string{
		"91.108.4.0/22",    // DC1
		"91.108.56.0/22",   // DC5
		"149.154.160.0/20", // NL Telegram CDN
		"91.105.192.0/23",  // RU-facing CDN
		"95.161.64.0/20",   // RU-facing CDN
	}
	gotCIDRsRaw, _ := ipRule["ip_cidr"].([]any)
	var gotCIDRs []string
	for _, c := range gotCIDRsRaw {
		if s, ok := c.(string); ok {
			gotCIDRs = append(gotCIDRs, s)
		}
	}
	for _, c := range wantCIDRs {
		if !slices.Contains(gotCIDRs, c) {
			t.Errorf("Telegram CIDR rule missing %q; got %v", c, gotCIDRs)
		}
	}
}

// findTelegramRule scans the rules slice and returns the first one whose
// domain_suffix or ip_cidr (selected by byDomain) contains a known Telegram
// fingerprint. Returns nil + -1 if not found.
func findTelegramRule(rules []any, byDomain bool) (map[string]any, int) {
	for i, r := range rules {
		m, ok := r.(map[string]any)
		if !ok {
			continue
		...
		if byDomain {
			if suffixes, ok := m["domain_suffix"].([]any); ok {
				for _, s := range suffixes {
					if str, _ := s.(string); str == "t.me" || str == "telegram.org" {
						return m, i
					}
				}
			}
			...
			if cidrs, ok := m["ip_cidr"].([]any); ok {
				for _, c := range cidrs {
					if str, _ := c.(string); str == "91.108.4.0/22" || str == "149.154.160.0/20" {
						return m, i
					}
				}
			}
